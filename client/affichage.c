...
{
    int cote = 75;
    cp[0] = initCase(0, 0, 0, 255, 0, 0, cote, cote); //Noir
    cp[1] = initCase(128, 128, 128, 255, 0, 0, cote, cote); //Gris
    cp[2] = initCase(255, 255, 255, 255, 0, 0, cote, cote); //Blanc
    cp[3] = initCase(165, 42, 42, 255, 0, 0, cote, cote); //Marron
    cp[4] = initCase(255, 0, 0, 255, 0, 0, cote, cote); //Rouge
    ...
    cp[10] = initCase(0, 0, 255, 255, 0, 0, cote, cote); //Bleu
    cp[11] = initCase(128, 0, 128, 255, 0, 0, cote, cote); //Violet
    cp[12] = initCase(255, 0, 255, 255, 0, 0, cote, cote); //Magenta
    cp[13] = initCase(255, 192, 203, 255, 0, 0, cote, cote); //Rose 
}
...
void draw_matrix(SDL_Renderer *renderer, int MatrixWidth, int MatrixHeight, CASE colorRect[MatrixHeight][MatrixWidth])
{
    for (int i = 0; i < MatrixHeight; i++)
    {
        for (int j = 0; j < MatrixWidth; j++)
        {
            ...
            SDL_SetRenderDrawColor(renderer, 0, 195, 195, 255);
